...
<Card.Root>
  ...
  >
    <Card.Title class="text-sm font-medium">{text} alerts</Card.Title>
      <p class="font-black" style={"color:"+color+";"} >{alerts[alerts.length-1]}</p>
    </Card.Header>
      <Card.Content
    class="flex w-full gap-2 h-[100px] items-end pb-0 mt-4 mb-0"
  >
    {#each alertsGraph as item, i}
      <div transition:slide={{delay: 500+i*100, duration: 1500}} 
      ...
      >
        <Tooltip.Provider>
          <Tooltip.Root>
            ...
            >
              <div
                style={"height:" + item.height + "px; background-color:"+color+";"}
