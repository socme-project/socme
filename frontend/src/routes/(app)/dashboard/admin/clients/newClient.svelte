<script lang="ts">
  ...
  let name = $state(""); // esiea
  let logo = $state(""); // https://www.esiea.fr/wp-content/themes/esiea/img/logo-esiea.svg

  let wazuhHost = $state(""); // 10.8.178.20:55000
  let wazuhUsername = $state(""); // admin
  let wazuhPassword = $state(""); // HMthisismys3cr3tP5ssword34a;

  let indexerHost = $state(""); // 10.8.178.20:9200
  let indexerUsername = $state(""); // admin
  let indexerPassword = $state(""); // HMthisismys3cr3tP5ssword34a;
    ...
    await axios
      ...
      .catch((error) => {
        if (error.response && error.response.data) {
          toast.error(
            "An unknown error occurred. " + error.response.data.error,
          );
        } else {
