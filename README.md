...
Un **OpenSOC** est une version open-source d'un SOC qui vise à offrir des solutions accessibles pour la détection et la réponse aux menaces. Il permet aux entreprises et aux chercheurs de mettre en place une infrastructure de sécurité sans coûts de licence élevés.
...
- Zeus:
  ...
  - OpenSOC:
    ...
    C'est cette application qui va communiquer avec les autres applications (Wazuh, TheHive, OpenStatus, ...)
...
- Artemis:
  ...
  - TheHive:
    ...
    Les alertes récolté par l'OpenSOC qui nous semblent suspectes seront envoyées à TheHive pour être traitées par les clients.
...
En sveltekit avec un backend golang (version golang).

Vas utiliser l'API de Wazuh pour récupérer les alertes, TheHive pour créer les incidents.
Vas utiliser des commandes système pour gérer les Artemis (headscale, ssh, ...): C'est donc le backend en golang qui va executer des os.exec(). Vas falloir bien géré les permissions pour que le backend est le droit d'utiliser tout ça
...
- [ ] Manage users
  ...
  - [ ] Settings
- [ ] Manage clients
  - [ ] Create headscale user on Creation
- [ ] Cron
  - [ ] Get alerts
  - [ ] Get clients status
- [ ] TV View
...
- [ ] Get from pocketbase API
- [ ] SSH Commands to Artemis
- [ ] Create basics commands:
  - [ ] Status
  - [ ] Update (nixos, git stash ++)
